...
import { getAllTopics, getArticlesByTopic, getTopicName, type TopicKey } from "@/lib/assets"
...
function NewsHomePage() {
  ...
  const filteredTopics = useMemo(() => {
    ...
    if (selectedCategory !== "all") {
      filtered = filtered.filter((topic) => topic.topicKey === selectedCategory)
    }

    // Filter by search query with enhanced search logic
    if (searchQuery.trim()) {
      ...
      filtered = filtered
        .map((topic) => ({
          ...
          articles: topic.articles.filter((article) => {
            ...
            const topicName = getTopicName(topic.topicKey, language).toLowerCase()
  ...
  const totalArticles = filteredTopics.reduce((acc, topic) => acc + topic.articles.length, 0)

  return (
    <div className="min-h-screen bg-background">
      <Navigation onSearch={handleSearch} onCategoryFilter={handleCategoryFilter} selectedCategory={selectedCategory} />
      ...
      <main className="container mx-auto px-4 py-6 sm:py-8 lg:py-12">
        ...
        <div className="mb-8 sm:mb-12 lg:mb-16 text-center">
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-foreground mb-4 sm:mb-6 text-balance leading-tight">
            {language === "en" ? "Stay Informed with Latest News" : "ताजा समाचारहरूसँग अपडेट रहनुहोस्"}
          </h1>
        ...
        {(searchQuery || selectedCategory !== "all") && (
          <div className="mt-8 sm:mt-12 p-4 sm:p-6 bg-gradient-to-r from-muted/30 to-muted/50 rounded-lg border border-border backdrop-blur-sm">
            <div className="text-center space-y-2">
              {searchQuery && (
                <p className="text-sm sm:text-base text-muted-foreground">
                  {language === "en" ? `Search results for "${searchQuery}"` : `"${searchQuery}" का लागि खोज परिणामहरू`}
                </p>
              )}
              {selectedCategory !== "all" && (
                <p className="text-sm sm:text-base text-muted-foreground">
                  {language === "en"
                    ? `Filtered by category: ${getTopicName(selectedCategory as TopicKey, language)}`
                    : `श्रेणी अनुसार फिल्टर गरिएको: ${getTopicName(selectedCategory as TopicKey, language)}`}
                </p>
              )}
              <p className="text-xs sm:text-sm text-muted-foreground font-medium">
                {language === "en" ? `${totalArticles} articles found` : `${totalArticles} लेखहरू फेला परे`}
              </p>
            </div>
          </div>
        )}
